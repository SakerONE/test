(function(){
	...
	function Pie(canvas, options){
		...
		// Animation direction. It depends on starting and ending angles.
	...
	Pie.prototype.animate = function(end, duration){
		...
		this.animation = setInterval(function(){
			stepIndex++;
			that.current += step;
			that.setTo(that.current);
			that.onstep(that.current);
			if(stepIndex === stepsNumber){
				that.current = end;
				that.onfinalstep(that.current);
				clearInterval(that.animation);
			}
		}, this.animationInterval);
	};
	Pie.prototype.setTo = function(end){
		this.context.clearRect(0, 0, this.canvas.width, this.canvas.height);
		this.context.beginPath();
		this.context.moveTo(this.xyr, this.xyr);
	    this.context.arc(this.xyr, this.xyr, this.xyr, this.start, end, this.counterclockwise);
		this.context.fill();
	};
